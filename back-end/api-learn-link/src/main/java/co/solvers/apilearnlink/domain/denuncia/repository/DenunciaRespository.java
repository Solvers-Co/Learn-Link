...
public interface DenunciaRespository extends JpaRepository<Denuncia, Long> {
    ...
    /*    @Query("SELECT new co.solvers.apilearnlink.service.publicacoesDenunciadas.dto.PublicacoesDenunciadasDto(" +
            "p, COUNT(d.id)) " +
            "FROM Denuncia d JOIN d.publicacao p " +
            "GROUP BY p.id")
    List<PublicacoesDenunciadasDto> buscaPublicacoesDenunciadas();*/
